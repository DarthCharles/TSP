...
public class Matrix extends javax.swing.JDialog {
    ...
    public Matrix(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        this.setTitle("TSP - Matriz de Adyacencias");
        this.setLocationRelativeTo(null);

        double array[][] = new double[Mapa.Cuidades.size()][Mapa.Cuidades.size()];
        double distancia = 0;
        TSP_ALGM.get_matrix(distancia, array, Mapa.Cuidades);
        DefaultTableModel modelo = (DefaultTableModel) tablis.getModel();

        // tabla.getColumnModel().getColumn(0).setPreferredWidth(15);
        modelo.addColumn(" ");
        for (int i = 0; i < modelo.getColumnCount(); i++) {
            tablis.getColumnModel().getColumn(0).setMinWidth(50);
        }


        for (int i = 0; i < Mapa.Cuidades.size(); i++) {
            modelo.addRow(new Object[0]);
            modelo.addColumn(Mapa.Cuidades.get(i).nombre);


        }
        for (int i = 0; i < modelo.getRowCount(); i++) {
            modelo.setValueAt(Mapa.Cuidades.get(i).nombre, i, 0);
        }
        for (int i = 0; i < modelo.getRowCount(); i++) {
            for (int j = 0; j < modelo.getColumnCount() - 1; j++) {


                modelo.setValueAt("$" + (int) array[i][j], i, j + 1);
            }
        }
    }
    ...
    private void initComponents() {
        ...
        tablis.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);
        tablis.setEnabled(false);
